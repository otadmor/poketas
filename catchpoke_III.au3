...
Func get_stat_hp_value($level, $base, $iv, $ev)
...
Func is_valid_hp_stat($level, $base, $ev, $stat)
   ...
   if $level <= 50 Then
	  return True
   EndIf
   if Ceiling((($stat - 10 - $level) * 100) / $level) <> Floor((($stat - 10 - $level) * 100 + 99)/ $level) Then
	  return False
   EndIf
   return True
...
Func get_hp_dv_value_min($level, $base, $ev, $stat)
   if not is_valid_hp_stat($level, $base, $ev, $stat) Then
	  return False
   EndIf
    Return _Max(_Min(Floor( ($stat - 10 - $level) * 100       / $level - 2 * $base - Floor($ev / 4)), 31), 0)
EndFunc
...
Func get_hp_dv_value_max($level, $base, $ev, $stat)
   ...
   if not is_valid_hp_stat($level, $base, $ev, $stat) Then
	  return False
   EndIf
    Return _Max(_Min(Ceiling  ((($stat - 10 - $level) * 100 + 99) / $level - 2 * $base - Floor($ev / 4), 31), 0)
 EndFunc

Func get_stat_value($level, $base, $dv, $ev, $nature)
	Return Floor(Floor((((2 * $base + $iv + Floor($ev / 4)) * $level) / 100) + 5) * $nature)
 EndFunc

Func is_valid_stat($level, $base, $ev, $stat, $nature)
   if $stat < 5 Then
	  return False
   EndIf
   if $level <= 50 Then
	  return True
   EndIf
   If Ceiling((($stat / $nature - 5) * 100) / $level) <>  Floor((($stat / $nature - 5) * 100 + 99)/ $level) Then
	  return False
   EndIf
   return True
...
Func get_dv_value_min($level, $base, $ev, $stat, $nature)
   if not is_valid_stat($level, $base, $ev, $stat) Then
	  return False
   EndIf
    Return _Max(_Min(Floor( ($stat / $nature - 5 ) * 100       / $level - 2 * $base      - Floor($ev / 4)), 31), 0)
EndFunc
Func get_dv_value_max($level, $base, $ev, $stat, $nature)
   if not is_valid_stat($level, $base, $ev, $stat) Then
	  return False
   EndIf
    Return _Max(_Min(Ceiling  ((($stat / $nature - 5 ) * 100 + 99) / $level - 2 * $base      - Floor($ev / 4)), 31), 0)
EndFunc

Func get_max_hp($level, $base)
	Return get_stat_hp_value($level, $base, 31, 0)
EndFunc

Func get_max_stat($level, $base, $nature)
	Return get_stat_value($level, $base, 31, 0, $nature)
EndFunc


Func get_mid_stat($level, $base, $nature)
	Return get_stat_value($level, $base, 31, 0, $nature)
EndFunc
